...
**対象リビジョン**: リポジトリ内のコード（確認日: 2026-05-17）  
...
| ソーシャル | フォロー申請・承認/否認、フレンド一覧、フォロー中/フォロワー。**新規フォローの開始は招待URLの共有またはQR読み取りのみ**（ユーザー名等の検索は提供しない）。QR/招待はユーザーIDに紐づく |
| スレッド | フレンドの `show_in_timeline` かつ7日以内の投稿、RPC `get_timeline_posts` 取得 |
| メモリ再浮上 | スレッド先頭に「過去の今日の投稿」を表示（プッシュ通知なし・本人のみ閲覧可）。フリープラン: 1年前の同日。Plusプラン: 1ヶ月/3ヶ月/6ヶ月/1年/3年/5年前の同日。RPC `get_thread_memory_resurface` 取得。 |
| サブスクリプション（Plus） | RevenueCat 経由の IAP（iOS）。`subscriptions` テーブルで状態管理。`is_current_user_premium()` 関数でプラン判定。有効ステータス: `active` / `in_grace_period` / `in_billing_retry`。 |
| 反応 | 定義された絵文字5種のリアクション（投稿あたり自分1種） |
| 設定 | 言語（ja/en）、表示設定、各種静的画面（利用規約・プライバシー等） |

### 1.2 本書のスコープ外・レガシー

- **ユーザー検索機能（名前・メール・キーワードなどによる他ユーザーの探索）は提供しない**。新規にフォロー関係を作る導線は **招待URL の共有** および **QR コード読み取り** のみ。旧ドキュメントの文言検索ベース探索は要件としない。  
...
- **RPC** `get_timeline_posts`（`SECURITY INVOKER`、マイグレーション `20260437_*` が最新定義）を `supabase.rpc` で呼び出し、**直近7日**・**`show_in_timeline`**・**非論理削除**の投稿を返す。  
- フレンド**のみ**（相互かつ承認済み; マイグレーション定義に準拠）。

### 3.3 メモリ再浮上（過去の今日の投稿）

- **目的**: ユーザーが過去の同日の投稿をスレッド画面先頭で見返せる機能。プッシュ通知なし・本人のみ表示。
- **RPC** `get_thread_memory_resurface(p_client_tz text)`（`SECURITY DEFINER`、マイグレーション `20260510_*`）。
- **表示条件**:

| プラン | 対象ホライズン |
|---|---|
| フリー | 1年前の今日（`1Y`）のみ |
| Plus | 1ヶ月（`1M`）/ 3ヶ月（`3M`）/ 6ヶ月（`6M`）/ 1年（`1Y`）/ 3年（`3Y`）/ 5年（`5Y`）前の今日 |

- **Plusプラン判定**: `public.is_current_user_premium()` 関数（`subscriptions` テーブル参照）。
- **表示期間**: 当日のみ（RPC が `date_logged::date = (today - interval)::date` で日付一致を判定するため、翌日以降は自動的に消える）。
- **クライアントTZ**: `Intl.DateTimeFormat().resolvedOptions().timeZone` を `p_client_tz` に渡す。
- **複数表示**: 同日に複数ホライズンが重なる場合、またはホライズン内に複数投稿がある場合は、すべてスレッド先頭に並べて表示する。
- **該当なし**: 一致する投稿がなければ何も表示しない（タイムラインは通常通り）。
- **取得タイミング**: スレッド画面フォーカス時・プルリフレッシュ時に毎回 RPC を呼び出す（同日内は何度でも表示される）。

### 3.4 サブスクリプション（Plus プラン）

- **テーブル**: `public.subscriptions`（マイグレーション `20260504_*`）。
- **有効ステータス**: `active` / `in_grace_period` / `in_billing_retry`（かつ `expires_at` が未来または NULL）。
- **判定関数**: `public.is_current_user_premium()`（`SECURITY DEFINER`）— RPC・RLS から参照可能。
- **同期**: App Store Server Notifications V2 → Edge Function `revenuecat-webhook` → `subscriptions` テーブル更新。
...
- 旧ドラフト由来の **`UserSearch` ／独立 `FollowingList` 単独ルート**は採用しておらず `AppNavigator` に**未登録**。**ユーザー検索画面は実装しない**方針。`FollowListScreen.js` 等はレガシー残ファイルの可能性あり。  
...
| 項目 | 内容 |
|------|------|
| 日々の起動 | `npm run start`（Dev Client）。Expo Go では AdMob / IAP が動かないため、必ず Dev Client ビルドで開発する。 |
| 本番/検証用ビルド | **EAS Build** を採用（`eas.json` あり）。プロファイルは `development` / `preview`（TestFlight）/ `production`（App Store 提出）。 |
| 環境変数 **注入** | クライアント用は `EXPO_PUBLIC_*`。**EAS Secrets** に環境別登録（`EXPO_PUBLIC_SUPABASE_URL`、`EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY`、`EXPO_PUBLIC_SERVER_URL`、`ADMOB_*` 等）。 |
...
| 項目 | 内容 |
|------|------|
| クライアント | **専用のクラッシュ SDK は未導入。** TestFlight / App Store Connect のクラッシュレポートおよびユーザー報告を参照。 |
| サーバ | Supabase Edge Function ログ → **Better Stack Logs** に Log Drain で転送。重要メトリクスは Slack #ops 通知。 |
| 連絡体制 | SEV1/2/3 を `docs/ops-monitoring.md` §6 に定義。状態ページは Better Stack Status を採用予定。 |
...
| 種別 | 値 |
|------|-----|
| クライアント | App Store: `https://apps.apple.com/jp/app/id6793158649` |
| 招待中継 | Edge Function `invite-redirect`。`https://<prod-ref>.supabase.co/functions/v1/invite-redirect/<userId>`。Supabase 共有ドメインは HTML を返せない（content-type が `text/plain` に書き換えられる）ため、URL 末尾の `userId` を取り出して中継ページへ 302 する。遷移先はシークレット `INVITE_LANDING_URL`、userId が取れない場合の退避先は `IOS_STORE_URL`。 |
| 招待中継ページ | `docs/invite/index.html` を GitHub Pages（`main` ブランチ `/docs`）で配信。`https://kyota1210.github.io/DaiB-app2026/invite/?u=<userId>`。「アプリで開く」（`daibapp://invite/<userId>`）と App Store の2択を提示する。 |
| 深いリンク | `app.json` `scheme: daibapp`。受け口は `client-app/App.js` の linking config（`InviteHandler: 'invite/:userId'`）。Universal Links は未対応（AASA を置ける独自ドメインがないため）。 |
| 状態ページ | Better Stack Status（公開後に確定）。 |
| 法務文書 | 正本は GitHub Pages（`https://kyota1210.github.io/daib-legal/`）。プライバシー / 利用規約 / 特商法。アプリは `LEGAL_URLS`（`client-app/config.js`）経由で外部ブラウザ表示。リポジトリ内 `docs/privacy-policy.md` は公開ページのミラー。 |
...
| `docs/ops-monitoring.md` | Better Stack / 状態ページ運用 | 高 |
...
| 運営者情報（特商法表記） | 公開ページ `https://kyota1210.github.io/daib-legal/specified-commercial.html` を正本とする | 高 |
...
| 2026-05-09 | §1.1 にメモリ再浮上・サブスクリプション（Plus）を追加。§3.3 メモリ再浮上・§3.4 サブスクリプション を新設。ER図に `subscriptions` を追加。 |
| 2026-05-17 | §1.1 / §1.2 / §4.3 に「ユーザー検索は提供しない。新規フォロー開始は招待URL・QRのみ」を明示。 |
