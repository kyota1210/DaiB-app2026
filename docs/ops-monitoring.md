...
DaiB iOS 本番リリース後の運用監視・アラートに関する設定手順をまとめる。クライアントのクラッシュ監視ツール（Sentry 等）は現状未導入とし、Supabase のログ・メトリクスおよびストア提供のレポートを起点に、最低限のアラート体制を構築する。
...
| レイヤー | ツール | 取得対象 |
|---|---|---|
| クライアント | App Store Connect（クラッシュレポート）／ユーザー報告 | ネイティブクラッシュの傾向（Sentry 等は未導入） |
...
## 2. Supabase ログ転送（Logflare / Better Stack）

Supabase は標準で 7 日間ログ保持。長期保管・全文検索のため外部 sink に転送する。

### 2.1 Better Stack Logs（推奨）
...
4. Better Stack 側で以下のアラートを作成：
   ...
   - `metadata.function_id = "revenuecat-webhook" AND status = 500` が 10 分で 3 件超 → 重大
...
### 2.3 Edge Function ログのフォーマット

`console.log` / `console.error` で出力すれば自動収集される。エラー時は `JSON.stringify` で構造化して出すと検索しやすい：

```typescript
console.error(JSON.stringify({ scope: 'revenuecat-webhook', step: 'upsert', err: String(e), userId }));
```

既存 Edge Functions にも段階的に適用する（F3-2 後続）。
...
### 3.2 アラート（Better Stack Heartbeats などで擬似的に）
...
## 4. App Store Connect / AdMob

### 4.1 App Store Connect

- App Analytics → 売上・サブスク状況を週次レビュー。
- RevenueCat Webhook のリトライが無限に続くケースはダッシュボードと `revenuecat-webhook` のエラーログ（§2 の Log Drain）で監視する。
...
- Heartbeat（§3.2）が 5 分以上失敗 → 自動で degraded、3 分以上 → 自動で down。
- メンテ予告は手動投稿。
...
連絡先・運用窓口は `docs/operations-info.md` の「24 時間以内通報対応」運用と統合する。インシデント発生時は **状態ページの更新 → ログ・ストアのレポートでの原因特定 → ロールバック判断（EAS の Update Channel または App Store の Phased Release 一時停止）** の順。
...
- [ ] Slack #ops チャンネルを作成し、Better Stack の Webhook を接続
