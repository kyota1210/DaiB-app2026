...
| RevenueCat Webhook 連携失敗 | `tag:operation:revenuecat-webhook AND level:error` が 5 分で 3 件超 | Slack #ops（重大） |
...
- RevenueCat Webhook のリトライが無限に続くケースはダッシュボードと `revenuecat-webhook` のエラーログ（Sentry）で監視する。
