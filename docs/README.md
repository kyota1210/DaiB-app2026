...
DaiB（デイビー）は、ユーザーが日々の記録を写真と共に作成・管理できるモバイルアプリケーションです。JWT認証により、各ユーザーは自分の記録のみにアクセスでき、セキュアな環境で記録を管理できます。
...
└── Doc/                    # データベース設計ファイル
    └── DDL/
        └── daib.a5er     # A5:SQL Mk-2 データベース設計ファイル
```
...
```sql
CREATE DATABASE daib CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE daib;
```
