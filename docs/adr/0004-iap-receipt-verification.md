...
DaiB でPlusプラン（月額サブスクリプション）を提供するにあたり、**サーバー側で `public.subscriptions` を権威とする**必要がある。クライアントのみの判定はなりすましやサーバー側機能ゲートと両立しない。
...
4. **アプリ内のPlusプラン判定**: `client-app/context/SubscriptionContext.js` が引き続き `subscriptions` を読む（**Supabase 正**。実装フラグ名は `isPremium`）。
