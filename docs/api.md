...
> **プロダクト要件**: DaiB は **ユーザー検索機能を提供しない**（新規フォロー開始は招待URL・QRのみ）。以下は **`server-api` の `/api` が廃止（410）前の歴史的ドラフト**。現行実装では当該エンドポイントは応答しない（廃止応答）。

ユーザー検索（旧ドラフト: 公開は部分一致、非公開は検索キー完全一致時のみ）
